...
export const Profile: FC<Props> = ({ signOut }) => {
  ...
  const onClickSubmitDisplayName = (name?: string | number) => {
    return updateUser({ ...user, displayName: String(name) || "" })
  }
  const onClickSubmitAge = (age?: string | number) => {
    return updateUser({ ...user, age: Number(age) })
  }
  ...
  const lastPlayed = useMemo(() => {
    ...
    return user?.lastTestAt
      ? moment
          .duration(
            moment(currentTimestamp, "HH:mm:ss").diff(
              moment(user?.lastTestAt?.toDate(), "HH:mm:ss")
            )
          )
          .humanize() + " ago"
      : "No tests yet"
      ...
      <Grid spacing={2} container>
        ...
        <Grid xs={6} item>
          ...
          >
            <Grid sx={{ alignItems: "center" }} container>
              <Grid xs={9} item>
                <Typography variant="caption">Last test</Typography>
        ...
        <Grid xs={6} item>
          ...
          >
            <Grid sx={{ alignItems: "center" }} container>
              <Grid xs={9} item>
                <Typography variant="caption">Deviation</Typography>
                {user?.score && (
